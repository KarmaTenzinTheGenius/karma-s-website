...
function updateDashboard() {
  ...
  document.getElementById('salesData').textContent =
    `Total: $${totalRevenue.toFixed(2)} from ${allOrders.length} orders`;
...
function viewOrderDetails(orderId) {
  ...
  const itemsList = order.items.map(item => `
    <div class="order-detail-item">
      ...
      <span class="order-detail-item-text">
...
function updateCategoryStats() {
  ...
  const categoryData =
    document.getElementById('categoryData');
...
function editProduct(productId) {
  ...
  const newPrice = prompt(
    `Edit price for "${product.name}":\nCurrent: $${product.price}`,
    product.price
  );

  if (newPrice !== null && !isNaN(newPrice)) {
    alert(
      `✓ Price for "${product.name}" updated to $${newPrice}\n\n` +
