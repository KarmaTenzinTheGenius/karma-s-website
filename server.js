...
// In-memory orders storage (in production, use a database)
let orders = [];
  ...
  try {
    ...
    if (!productId || !customerName || !rating || !title) {
      return res.status(400).json({ 
        success: false, 
        error: "productId, customerName, rating, and title are required" 
      });
    }
...
// ========== RECOMMENDATIONS & PROMOTIONS API ROUTES ==========

// 13. GET TOP RATED PRODUCTS
app.get('/api/top-rated', (req, res) => {
  try {
    ...
    // Sort by rating (only show products with reviews)
  ...
  try {
    ...
    const discounted = products
      ...
      .map(p => ({
        ...
        discountedPrice: (p.price * (1 - p.discount / 100)).toFixed(2),
        savings: (p.price * p.discount / 100).toFixed(2)
      }));
...
// 15. GET PRODUCT RECOMMENDATIONS (based on category)
...
// 16. GET FEATURED SUGGESTIONS (combination of top-rated and popular)
app.get('/api/suggestions', (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 8;
    
    // Combine top-rated with some random products for variety
    const productRatings = products.map(product => {
      const productReviews = reviews.filter(r => r.productId === product.id);
      const avgRating = productReviews.length > 0
        ? productReviews.reduce((sum, r) => sum + r.rating, 0) / productReviews.length
      ...
      return {
        ...
        rating: parseFloat(avgRating.toFixed(1)),
        reviewCount: productReviews.length,
        score: avgRating * 0.7 + (productReviews.length * 0.3) // Weight rating and review count
      };
    });
    
    // Sort by score and get top suggestions
    const suggestions = productRatings
